...
public class WaterchangeController {
	...
	public String save(HttpServletRequest request,WaterChangeModel c,String oldwatermetercode)
	{
		...
		try {
			...
			String deviceId = opentionEquipmentNBLOT(addEquipment);
			if(!"".equals(deviceId) && deviceId != null){
				addEquipment.setDeviceid(deviceId);
				equipmentService.opentionEquipment(addEquipment);
			}
			
			String typeId = addEquipment.getWatermeterid();
			
			/*				nb_watermeter_dict watermeter = watermeterService.infoWatermeter(typeId);
			
		if("0".equals(watermeter.getMagnetism())){
				//无磁设置表读数命令
				signalDeliveryService.readBasicNumTLV(deviceId,addEquipment.getBasenum(),"SETRAW");
			}else{
				//有磁设置表读数命令
				signalDeliveryService.readBasicNumHAC(deviceId,addEquipment.getBasenum(),nbIotConfig.getWater_meter_basic());
			}*/
	...
	/**
	 * 注册NB平台设备
	 * @param equipment 水表对象
	 * @return 返回水表水表ID
	 */
	private String opentionEquipmentNBLOT(nb_watermeter_equipment ement){
		String deviceId = "";
		try {
			
			Map<String,Object> map = new HashMap<String,Object>();
			map.put("customerId", ement.getCustomercode());
			map.put("id", ement.getId());
			EquipmentModel equipment = equipmentService.infoEquipmentM(map);//查询水表设备信息
			...
			if(equipment.getDeviceid() == null || "".equals(equipment.getDeviceid())){
				RegDirectDeviceOutDTO rddod = deviceManagementService.registerDevice(deviceManagement, 3000,equipment.getWatermetercode());
	...
	private nb_watermeter_equipment setAddEquipment(WaterChangeModel c,String customerId){
		...
		NorthApiClient cilent = intiClient.GetNorthApiClient();
		DeviceManagement deviceManagement = new DeviceManagement(cilent); //设备管理类
		deviceManagementService.deleteDirectDevice(deviceManagement, equipmentDB.getDeviceid());
		return equipment;
	}
}
